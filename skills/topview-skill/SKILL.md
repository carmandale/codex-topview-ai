---
name: topview-skill
description: "Official Topview AI client and Codex plugin orchestrator. Use for Topview video, image, avatar, voice, board, marketing-research, scriptwriting, and finished-video publishing workflows. All Topview API network calls go to *.topview.ai only."
license: Apache-2.0
metadata:
...
This plugin combines the official Topview client with focused sibling skills for research, scriptwriting, and publishing:

- Use `multi-platform-content-collector` for public web or social research, including competitor content, creators, comments, trends, links, and prompts. Preserve source URLs and separate observed facts from creative recommendations.
- Use `video-script-writer` before generating a new video from a broad creative brief. Prepare the angle, hook, script, shot flow, storyboard, and model-ready prompt; show the draft and wait for the user's approval unless they explicitly ask to skip review.
- After script approval, ask in the user's language whether they want an optional Storyboard preview image from `ai_image.py --type storyboard`. If they skip it, continue directly to video generation.
- When an approved Storyboard preview is used, run `video_gen.py --type omni` with `--storyboard-image`, all product or user reference images in `--input-images`, and the approved script in `--prompt`.
- Use `social-media-uploader` only after a finished video exists. Prefer a `--no-publish` dry run for the first upload in a session, and never publish without explicit user confirmation of the platform settings and final file.
- Topview sign-in uses the direct chat link from `auth.py`. If a social platform session is not ready, follow `social-media-uploader` and ask the user to sign in only in their own visible Chrome debug browser; never request or handle their password.

For end-to-end content work, follow this sequence: research, script, optional Storyboard preview, generate, organize results on a board, then publish. Do not block generation when the user declines the optional preview.
...
6. **Wait for the user to confirm sign-in** — ask the user to reply "done", then continue.
...
| AI Image | `scripts/ai_image.py` | [ai_image.md](references/ai_image.md) | Text-to-image, AI image editing, and fixed Storyboard grid previews |
...
| "Preview an approved script as a storyboard grid" | `ai_image.py --type storyboard` |
