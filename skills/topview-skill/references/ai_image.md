...
Generate images from text prompts, edit existing images, or create grid Storyboard previews.
...
| `storyboard` | **Storyboard Preview** - generate a fixed 16:9 grid for an approved video beat | `--prompt`; optional cell ratio, shot count, duration, and references |
...
Use `storyboard` to generate one approved planning image that contains a chronological grid of shots. Backend parameters are fixed to `type="storyboardToVideo"`, `model="GPT Image 2"`, `aspectRatio="16:9"`, `resolution="2K"`, and `generateCount=1`; do not ask the user to choose those values.

```bash
python {baseDir}/scripts/ai_image.py run \
  --type storyboard \
  --prompt "A 15-second product reveal: hook, close-up demo, proof, CTA" \
  --cell-aspect-ratio "9:16" \
  --shot-count 6 \
  --target-duration-seconds 15 \
  --reference-images product.png creator.jpg
```

Storyboard rules:

- The full preview is a 16:9 landscape grid; every internal cell keeps `--cell-aspect-ratio`.
- `--shot-count` accepts 1-16. Omit it for an automatic layout, normally 4-9 cells.
- Only a small sequence number is visible in each cell; no captions, dialogue, titles, watermarks, UI labels, or bottom bars.
- Reference images map in order to `@Image 1`, `@Image 2`, and later tags in the story.
- The optional `--target-duration-seconds` makes the planned shot durations sum to the requested total.
...
| Option | Description |
|--------|-------------|
| `--type` | `text2image`, `image_edit`, or `storyboard` (required) |
| `--model` | Model display name (required for text2image/image_edit; fixed for storyboard) |
| `--prompt` | Text prompt (required) |
| `--aspect-ratio` | Aspect ratio required for text2image/image_edit; fixed at `16:9` for storyboard |
...
| `--cell-aspect-ratio` | Internal Storyboard cell ratio; default `9:16` |
| `--shot-count` | Fixed Storyboard cell count from 1-16; omit for automatic layout |
| `--target-duration-seconds` | Target total duration represented by the Storyboard |
| `--reference-images` | Ordered Storyboard reference images |
...
- `storyboard` uses fixed backend parameters and always generates exactly one 2K landscape grid preview.
